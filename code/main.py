...
def reset_stage():
    ...
    if current_stage == 1:
        print(f"=== STAGE {current_stage} : Enemy 1 (대량) ===")
        # 🌟 5마리 -> 10마리로 증가
        # 간격도 150으로 좁혀서 촘촘하게 배치 (500 ~ 2000 구간 커버)
        for i in range(10):
            mob = Enemy(500 + i * 150, 90)
            game_world.add_object(mob, 1)
            enemys.append(mob)

        current_portal = Portal(2350, 90)
        game_world.add_object(current_portal, 1)

    elif current_stage == 2:
        print(f"=== STAGE {current_stage} : Enemy 2 (대량) ===")
        # 🌟 5마리 -> 10마리로 증가
        for i in range(10):
            mob = Enemy2(500 + i * 150, 90)
            game_world.add_object(mob, 1)
            enemys.append(mob)
        ...
        print(f"=== STAGE {current_stage} : BOSS FIGHT (부하들 등장) ===")

        # 🌟 보스 호위 병력 6마리 (Enemy 3 + Enemy2 3)
        for i in range(3):
            # Enemy 1 배치
            mob1 = Enemy(400 + i * 300, 90)
            game_world.add_object(mob1, 1)
            enemys.append(mob1)

            # Enemy 2 배치 (사이사이에)
            mob2 = Enemy2(550 + i * 300, 90)
